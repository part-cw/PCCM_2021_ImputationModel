...
# creating complete population mentation dataset
...
# Inducing and imputing datasets under biased sampling (without replacement) scheme using Abnormal SBP
# Applying 5-fold Cross validation to train the models and compute AUC values including Confidence Intervals


simulate_abnormal_weighted_Mentation <- function(the_complete_df, runs=10, m = 5, prop_missing = 0.5, weights_vector = c(0.5,0.5), missing_variable) {
  ...
  for(run in 1:runs) 
  {
    ...
    data_disc_ment = data_Mentation %>% drop_na()
    outcomes <- data_disc_ment[,'outcome']
    data_disc_ment <- data_disc_ment[,c('abnormal_Mentation','abnormal_SBP','abnormal_RR')]
    outcomes = factor(outcomes)
    disc_model <- train(y=outcomes,x=data_disc_ment,method="glm",family="binomial",
    ...
    complete_imputed <- data.frame(complete(imp_Mentation, "long", inc = FALSE))
    outcomes <- complete_imputed[,'outcome']
    complete_imputed <- complete_imputed[,c('abnormal_Mentation' , 'abnormal_SBP', 'abnormal_RR')]
    ...
    data_norm_mentation = data_Mentation %>% mutate_if(is.logical, replace_na, replace = FALSE)
    outcomes <- data_norm_mentation[,'outcome']
    outcomes = factor(outcomes)
    data_norm_mentation <- data_norm_mentation[,c('abnormal_Mentation' , 'abnormal_SBP', 'abnormal_RR')]
    norm_model <- train(y=outcomes,x=data_norm_mentation,method="glm",family="binomial",
                        trControl = trainControl(method="cv",number = 5,savePredictions=TRUE,classProbs=TRUE,summaryFunction=twoClassSummary))
...
mentation_cis <- lapply(prop_missing_list, function(x){
...
saveRDS(mentation_cis,"AUC_CIs_150Runs_Mentation.RDS")
